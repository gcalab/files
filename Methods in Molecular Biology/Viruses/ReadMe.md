...
> [!IMPORTANT]
> Save the current session in Chimera for future reference. Go to `File` then click `Save Session As...` or simply press `Ctrl + s`. Additionally you should save each aligned structure/model separately with coordinates relative to the reference structure.
